...
PM_WRITABLE = {
    ...
    "bankruptcy", "warning_due", "claim_due",
}
...
class AccountDetailSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        if "debt_group_manual" in validated_data and validated_data["debt_group_manual"] != instance.debt_group_manual:
            ...
            manual_notice = None
        turning_on = validated_data.get("inheritance_case") and not instance.inheritance_case
        ...
        if turning_on:
            main = instance.registrations.filter(subj_is_main=True).order_by("id").first()
            instance.inheritance_payer_id = main.subj_id if main else None
            instance.save(update_fields=["inheritance_payer_id", "updated_at"])
            from .services.portfolio import AUTO_MEASURES

            Measure.objects.filter(
                accounts=instance, kind__in=AUTO_MEASURES,
                status__in=[Measure.Status.ASSIGNED, Measure.Status.RUNNING],
            ).update(status=Measure.Status.CANCELLED)
        return instance
