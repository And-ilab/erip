...
На Windows `npm ci` внутри Docker обрывает демон с `error reading from server: EOF`. Фронт собирается на хосте, образ nginx только копирует `frontend/dist/erip/browser`. Образ backend ставит шрифт DejaVu: PDF предупреждения пишется кириллицей. CSV обзвона и вложения лежат в томе `media` и отдаются по `/media/`. Повторная сборка не стирает базу: при старте backend выполняет `migrate`.
...
```bash
cd app/backend && pytest --cov=apps        # 67 тестов
...
- Реестр и карточка договора (`/contracts`): услуга поставщика, суммы и пеня, дата возникновения, последняя оплата, своя группа. Поставщик не видит чужие услуги того же ЛС.
